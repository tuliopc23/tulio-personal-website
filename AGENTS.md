...
- Horizontal scroll rails/carousels use native overflow with `touch-action: pan-x pan-y` (or `pan-x pan-y pinch-zoom`) so vertical page scroll is not blocked on touch; register rails in `src/scripts/lenis.ts` (`data-lenis-prevent-horizontal`, `[data-case-track]`, etc.). Do not use `touch-action: pan-x` alone on rails — it traps vertical scroll. Avoid `data-lenis-prevent` on whole sliders and avoid Lenis `allowNestedScroll` for horizontal-only overflow — both trap vertical Lenis page scroll on desktop; use `virtualScroll` routing in `lenis.ts` instead.
