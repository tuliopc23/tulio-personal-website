describe("sanity image helpers", () => {
  test("uses Cloudflare transforms when configured", async () => {
    ...
    const { cloudflareImageUrl, generateSrcset } =
      await import("../../src/sanity/lib/image");
    ...
    expect(
      generateSrcset("https://cdn.sanity.io/images/demo/image.png", [320, 640])
    ).toContain(" 320w");
  });
  ...
  test("falls back to raw source without Cloudflare base", async () => {
    ...
    expect(
      optimizedImageUrl("https://cdn.sanity.io/images/demo/image.png", 1200, 630)
    ).toContain("auto=format");
  });
});
