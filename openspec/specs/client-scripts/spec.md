...
- **WHEN** building the project with `pnpm run build`
...
- **WHEN** running the dev server with `pnpm run dev`
...
- **WHEN** running `biome lint .` or `pnpm run lint`
...
- **WHEN** running `biome format .` or `pnpm run format:check`
...
- **WHEN** running CI checks with `pnpm run check:ci`
