...
- **pnpm 9** - Package manager (see `packageManager` in `package.json`); optional global [Vite+](https://viteplus.dev/) CLI (`vp`) for unified install/build/test
...
- **Biome 2.2.5** - Primary formatter & linter (`pnpm run lint`, `pnpm run format`, `pnpm run check`)
...
```bash
pnpm run lint             # Run Biome linting (non-destructive)
pnpm run lint:fix         # Apply Biome lint fixes
pnpm run format:check     # Dry-run Biome formatter
pnpm run format           # Format with Biome (writes)
pnpm run typecheck        # Run TypeScript compiler
pnpm run check            # Biome check (writes), typecheck, then build
```
...
```bash
pnpm exec sanity login    # Authenticate (one-time)
pnpm exec sanity deploy   # Deploy Studio to hosting
pnpm run sanity:typegen   # Generate TypeScript types from schema
```
...
- **Note:** Local dev falls back to the defaults above, but `pnpm run check` (production build) will fail if the variables are not explicitly set.
...
2. Run `pnpm run sanity:typegen` to update types
